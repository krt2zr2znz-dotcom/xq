...
for _code,_g in px.sort_values(['代碼','日期']).groupby('代碼'):
    ...
    for i in range(_n):
        ...
        if not ((zb or lb or zt or ztk or xh) and _k[i]>65): continue
        row={'d':d,'c':_code,'nm':_nm,'mk':_mk,'close':round(float(cl),2),'k':round(float(_k[i]),1),'chg':round(float(_ch[i]),2),'zt':zt,'zb':zb,'lb':lb,'ztk':ztk,'xh':xh}
