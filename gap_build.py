...
for _code,_g in px.sort_values(['代碼','日期']).groupby('代碼'):
    ...
    _cl=_g['收盤'].tolist();_op=_g['開盤'].tolist();_hi=_g['最高'].tolist();_lo=_g['最低'].tolist()
    ...
    for i in range(_n):
        ...
        _nb=bool(pd.notna(_bu[i]) and cl>=_bu[i]*0.95)   # ★回測也收到上軌下方5%(給可調臨近%)
        if not ((_nb or zt or ztk or xh) and _k[i]>65): continue
        row={'d':d,'c':_code,'nm':_nm,'mk':_mk,'close':round(float(cl),2),'k':round(float(_k[i]),1),'chg':round(float(_ch[i]),2),'zt':zt,'zb':zb,'lb':lb,'ztk':ztk,'xh':xh,'bu':round(float(_bu[i]),2) if pd.notna(_bu[i]) else None}
...
H=('<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
 ...
 '<div class="panel"><div class="presets">一鍵套用:<button onclick="preset(\'5.1\')">5.1</button><button onclick="preset(\'5.2\')">5.2</button><span style="color:#8aa1bd">符合 <span class="cnt" id="cnt">0</span> 檔</span></div>'
 '<div class="conds">'
 ...
 '<span class="num"><label class="ck"><input type="checkbox" id="gk">綠K量&ge;</label><input id="gkv" value="0"></span>'
 ...
 '<th onclick="sortBy(\'c\')">代碼</th><th>名稱</th><th onclick="sortBy(\'op\')">開盤</th><th onclick="sortBy(\'px\')">收盤</th><th onclick="sortBy(\'chg\')">漲跌%</th><th onclick="sortBy(&#39;vol&#39;)">量(張)</th><th onclick="sortBy(&#39;gvol&#39;)">綠K量</th>'
