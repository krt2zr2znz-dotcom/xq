...
for _,x in r.iterrows():
    ...
    data.append({'c':x['代碼'],'nm':str(x['商品']),'mk':str(x['交易所']),'op':round(float(x['開盤']),2),'px':round(float(x['收盤']),2),'chg':round(float(x['漲跌幅%']),2),'vol':round(float(x['成交量'])/1000) if pd.notna(x['成交量']) else None,
...
H=('<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
 ...
 '</div><div class="vbar"><button class="vbtn" id="bOnlyV">只看打V</button><button class="vbtn2" id="bClrV">清除全部V</button><span id="vcnt"></span><span style="flex:1"></span><span>匯入TradingView自選：</span><button class="vbtn2" id="bTvList">下載目前清單</button><button class="vbtn2" id="bTvV">下載打V</button><button class="vbtn2" id="bTvCopy">複製</button></div></div><div class="tb-scroll"><table><thead><tr><th class="l">V</th>'
 ...
 '<script>'+JS.replace('__DATA__',J).replace('__DATE__',json.dumps(DATE))+'</script></div></html>')
