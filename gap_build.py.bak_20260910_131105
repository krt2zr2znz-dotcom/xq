...
for _,x in r.iterrows():
    ...
    data.append({'c':x['代碼'],'nm':str(x['商品']),'op':round(float(x['開盤']),2),'px':round(float(x['收盤']),2),'chg':round(float(x['漲跌幅%']),2),'vol':round(float(x['成交量'])/1000) if pd.notna(x['成交量']) else None,
